...
# Parametros de configuração do cluster
...
with DAG(
    ...
    # Extração de dados via post API para o bucket raw na GCP
    get_data = SimpleHttpOperator(
        ...
        data=json.dumps({
            "bucket_name": "stack-data-pipeline-gcp-mg-combustiveis-brasil-raw",
    ...
    PYSPARK_JOB = {
        ...
        "pyspark_job": {
            ...
            "args": [
                '--path_input',
                "gs://stack-data-pipeline-gcp-mg-combustiveis-brasil-raw/combustiveis-brasil/{{ dag_run.logical_date.strftime('%Y') }}/{{ '01' if dag_run.logical_date.month <= 6 else '02'}}/ca-{{ dag_run.logical_date.strftime('%Y') }}-{{ '01' if dag_run.logical_date.month <= 6 else '02'}}.csv",
                '--path_output',
                "gs://stack-data-pipeline-gcp-mg-combustiveis-brasil-curated/combustiveis-brasil/{{ dag_run.logical_date.strftime('%Y') }}/{{ '01' if dag_run.logical_date.month <= 6 else '02'}}/",
                '--file_format', 'parquet',
